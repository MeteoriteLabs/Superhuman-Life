        ...
        [...flattenedData.clientBookings]
          ...
          .map((currValue) => {
            return {
              ...
              name:
                currValue.ClientUser && currValue.ClientUser.length
                  ? currValue.ClientUser[0].username && currValue.ClientUser[0].username
                  : null,
              email:
                currValue.ClientUser && currValue.ClientUser.length
              ...
              phone:
                currValue.ClientUser && currValue.ClientUser.length
                  ? currValue.ClientUser[0].Phone_Number && currValue.ClientUser[0].Phone_Number
                  : null
            };
          })
      );
    ...
    onCompleted: (data: any) => {
      ...
      setLeads(
        [...flattenedData.websiteContactForms].map((currValue) => {
          return {
            id: currValue.id,
            name: currValue.Details.leadsdetails.name && currValue.Details.leadsdetails.name,
            email: currValue.Details.leadsdetails.email && currValue.Details.leadsdetails.email,
            phone: currValue.Details.leadsdetails.contact && currValue.Details.leadsdetails.contact
          };
        })
      );
    }
