...
const AddFitnessAddressModal: React.FC<{ onHide: () => void; show: boolean }> = ({
  ...
  const validateAddress = async () => {
    try {
      const response = await axios.get(API_END_POINTS.googleGeoCodeValidation, {
        params: {
  ...
  return (
    <Modal show={show} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
      ...
      <Modal.Body>
        <Form.Group controlId="formBasicEmail">
          ...
          {/* {googleAddressShow && ( */}
            <GooglePlacesAutocomplete
              apiKey='AIzaSyDNzcfCY7aLfKP9f01E95ZopGgl-hh1074'
              selectProps={{
                value,
                onChange: (e: any) => {
                  setValue(e);
        ...
        {handleDisable() && (
          <span className="text-danger">It is compulsory to fill all the fields</span>
        )}
      </Modal.Body>
      ...
      <Modal.Footer>
        ...
        <Button variant="outline-success" onClick={handleAddressAdd} disabled={handleDisable()}>
          Add
        </Button>
      </Modal.Footer>
    </Modal>
