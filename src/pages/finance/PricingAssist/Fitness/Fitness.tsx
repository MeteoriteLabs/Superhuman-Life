...
export default function Fitness() {
      ...
      flattenFitnessPackages.fitnessPackageTypes
        ...
        .map((currValue: any) => {
          return currValue.Modes.Channel.map((channelMode: string[]) => {
            const indexOfPackage = flattenSuggestedPricing.suggestedPricings.findIndex(
              (element: any) =>
                element.Mode === channelMode && element.fitness_package_type.type === currValue.type
            );

            return {
              id:
                indexOfPackage !== -1
                  ? flattenSuggestedPricing.suggestedPricings[indexOfPackage].id
                  : '',
              ...
              mrp:
                indexOfPackage !== -1
                  ? flattenSuggestedPricing.suggestedPricings[indexOfPackage].mrp
                  : '--',
              updatedAt:
                indexOfPackage !== -1
                  ? moment(
                      flattenSuggestedPricing.suggestedPricings[indexOfPackage].updatedAt
                    ).format('MMMM DD,YYYY')
                  : '',
      ...
      { accessor: 'updatedAt', Header: 'Updated' },
      {
        ...
        Cell: ({ row }: any) => {
          const editPackagePricing = () => {
            pricingAssistAction.current.TriggerForm({
              id: row.original.id,
              actionType: 'edit',
  ...
  return (
    <div className="mt-5">
      <p>
        This base pricing will be used to help you price your offerings. It will suggest you the
        calculated price based mode, duration and type of offering.
      </p>
      <Row>
        <Col>
