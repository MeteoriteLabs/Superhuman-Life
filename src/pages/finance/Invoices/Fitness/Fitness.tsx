  ...
  const loadData = (data: any) => {
    setDataTable(
      [...data.clientBookings.data].map((fitnessPackage) => {
        const renewDay: Date = new Date(
          fitnessPackage.attributes.effective_date
        );
        renewDay.setDate(
          renewDay.getDate() + fitnessPackage.attributes.package_duration
        );
        const mrpFilter =
          fitnessPackage.attributes.fitnesspackages.data[0].attributes.fitnesspackagepricing.filter(
            (item) =>
              item.duration === fitnessPackage.attributes.package_duration
          );
        ...
        return {
          ...
          type: fitnessPackage.attributes.fitnesspackages.data[0].attributes
            .fitness_package_type.data.attributes.type,

          bookingDate: moment(fitnessPackage.attributes.booking_date).format(
            "MMMM DD,YYYY"
          ),
          mode: fitnessPackage.attributes.fitnesspackages.data[0].attributes
            .mode,
          packageName:
            fitnessPackage.attributes.fitnesspackages.data[0].attributes
              .packagename,
          duration: fitnessPackage.attributes.package_duration,
          effective: moment(fitnessPackage.attributes.effective_date).format(
            "MMMM DD,YYYY"
          ),
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }: any) => {
          const actionClick1 = () => {
            InvoiceActionRef.current.TriggerForm({
              id: row.original.id,
              actionType: "view",
