...
const ClientDetailsCard: React.FC<{
  Client: { First_Name: string; Last_Name: string; Phone_Number: string; email: string; id: string|number;}[];
}> = ({ Client }) => {
    ...
    <Col lg={3} sm={12}>
      <Card className="border p-3 rounded mt-5" style={{height: "60%"}}>
        <div className="d-flex">
          ...
          <div className="col-lg-11 col-sm-1" onClick={() => setShowClientModal(true)} style={{ cursor: 'pointer' }}>
            <Icon name="edit" />
          </div>
        </div>
        ...
        {showClientModal ? (
          <EditClient show={showClientModal} onHide={() => setShowClientModal(false)} Client={Client} />
        ) : null}
      </Card>
    </Col>
  );
