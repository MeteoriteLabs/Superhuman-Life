...
export default function PaymentSchedule() {
          ...
          () => [
               ...
               { accessor: "paymentdate", Header: "Payment Date" },
               {
                    accessor: "status", Header: "Status", Cell: ({ row }: any) => {
                         ...
                         return <>
                              <Badge className='px-3 py-1' style={{ fontSize: '1rem', borderRadius: '10px' }} variant={statusColor}>{row.values.status === "Activated" ? "Activated" : "Deactivated"}</Badge>
                         </>
                    }
               },
               {
     ...
     function loadData(data: any) {
          ...
          // setData([...flattenData.paymentSchedules]);
          setDataTable(
               [...flattenData.paymentSchedules].flatMap((Detail) => {
                    // let paymentDate: any = moment(Detail.Payment_DateTime);
                    // let reminderDate: any = moment(Detail.Reminder_DateTime);
                    // let diff = reminderDate.diff(paymentDate)
                    return {
                         id: Detail.id,
                         category: Detail.PaymentCatagory,
                         paymentdate: Detail.Payment_DateTime ? moment(Detail.Payment_DateTime).format('MMMM DD,YYYY') : '-',
                    ...
                    <Row>
                         ...
                         <Col>
                              <Card.Title className="text-center">
                                   ...
                                   <CreatePaymentSchedule ref={createPaymentScheduleComponent} callback={refetchQueryCallback}></CreatePaymentSchedule>
                              </Card.Title>
                         </Col>
                    </Row>
               </Container>
