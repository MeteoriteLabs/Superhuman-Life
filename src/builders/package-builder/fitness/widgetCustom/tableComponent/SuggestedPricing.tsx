...
export default function SuggestedPricing({
  ...
  const arrayDuration = fitnesspackagepricing?.map(
    (fitness) => fitness.duration
  );
  ...
  const calculateArraySuggestPrice = (
    partnerMRP: number,
  ...
  const PTSuggestedPricing = (data: any) => {
    ...
    const arrayPTdata = data.suggestedPricings.filter(
      (item) => item.fitness_package_type.type === "One-On-One"
    );
    const arrayPTClasses = [ptonline, ptoffline];

    const partnerSuggestPrice = calculateSuggestPrice(
      arrayPTdata,
      arrayPTClasses
    );
  ...
  const groupSuggestedPricing = (data) => {
    const arrayGroupData = data.suggestedPricings.filter(
      (item) => item.fitness_package_type.type === "Group Class"
    );
  ...
  const classicSuggestPricing = (data) => {
    const arrayClassicData = data.suggestedPricings.filter(
      (item) => item.fitness_package_type.type === "Classic Class"
    );
