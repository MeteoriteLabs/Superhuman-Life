...
function CreateEditView(props: any, ref: any) {
  ...
  const [packageTypeName, setPackageTypeName] = useState<string | null>(
    "personal-training"
  );
  const [actionName, setActionName] = useState<string>("");
  const [formData, setFormData] = useState<any>();
  const [sapienFitnessPackageTypes, setSapienFitnessPackageTypes] =
    useState<any>([]);
    ...
    <>
      ...
      {showStatusModal && (
        <StatusModal
          ...
          buttonRight="Yes"
          onClick={() =>
            TogglePackageStatus(operation.id, operation.current_status)
          }
        />
      )}
