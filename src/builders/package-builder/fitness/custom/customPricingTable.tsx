...
const PricingTable: React.FC<{
  ...
  const [pricing, setPricing] = useState<any>(
    props.value !== undefined && props.value !== 'free'
      ...
      : [
          ...
          { mrp: null, suggestedPrice: null, voucher: 0, duration: 90, sapienPricing: null },
          { mrp: null, suggestedPrice: null, voucher: 0, duration: 180, sapienPricing: null },
          { mrp: null, suggestedPrice: null, voucher: 0, duration: 360, sapienPricing: null }
        ]
  );
  ...
  const SUGGESTED_PRICING = gql`
    query fetchSapienPricing($id: ID!) {
      suggestedPricings(
        filters: {
          fitness_package_type: { type: { in: ["One-On-One", "Classic Class", "Group Class"] } }
  ...
  function handleSuggestedPricingCalculation(
    ...
    if (mode === 'Online') {
      ptOnlinePrice =
        suggestedPricings[
          suggestedPricings.findIndex((x: any) => x.fitness_package_type.type === 'One-On-One')
        ]?.mrp *
        ptOnlineClasses *
        (duration / 30);
      groupOnlinePrice =
        suggestedPricings[
          suggestedPricings.findIndex((x: any) => x.fitness_package_type.type === 'Group Class')
        ]?.mrp *
        groupOnlineClasses *
        (duration / 30);
      classicPrice =
        suggestedPricings[
          suggestedPricings.findIndex((x: any) => x.fitness_package_type.type === 'Classic Class')
        ]?.mrp *
        recorded *
        (duration / 30);
      ...
      ptOnlinePrice =
        suggestedPricings[
          suggestedPricings.findIndex(
            (x: any) => x.fitness_package_type.type === 'One-On-One' && x.Mode === 'Online'
          )
        ]?.mrp *
        ptOnlineClasses *
        (duration / 30);
      groupOnlinePrice =
        suggestedPricings[
          suggestedPricings.findIndex(
            (x: any) => x.fitness_package_type.type === 'Group Class' && x.Mode === 'Online'
          )
        ]?.mrp *
        groupOnlineClasses *
        (duration / 30);
      ptOfflinePrice =
        suggestedPricings[
          suggestedPricings.findIndex(
            (x: any) => x.fitness_package_type.type === 'One-On-One' && x.Mode === 'Offline'
          )
        ]?.mrp *
        ptOfflineClasses *
        (duration / 30);
      groupOfflinePrice =
        suggestedPricings[
          suggestedPricings.findIndex(
            (x: any) => x.fitness_package_type.type === 'Group Class' && x.Mode === 'Offline'
          )
        ]?.mrp *
        groupOfflineClasses *
        (duration / 30);
  ...
  function handleSapienPricingCalculation(
    ...
    if (mode === 'Online') {
      ptOnlinePrice =
        sapienPricings[
          sapienPricings.findIndex((x: any) => x.fitness_package_type.type === 'One-On-One')
        ].mrp *
        ptOnlineClasses *
        (duration / 30);
      const groupOnlinePrice =
        sapienPricings[
          sapienPricings.findIndex((x: any) => x.fitness_package_type.type === 'Group Class')
        ].mrp *
        groupOnlineClasses *
        (duration / 30);
      const classicPrice =
        sapienPricings[
          sapienPricings.findIndex((x: any) => x.fitness_package_type.type === 'Classic Class')
        ].mrp *
        recorded *
        (duration / 30);
      return ptOnlinePrice + groupOnlinePrice + classicPrice;
  ...
  function loadData(data) {
    ...
    if (classMode === 'Online') {
      ...
      flattenData.suggestedPricings = flattenData.suggestedPricings.filter(
        (item) => item.Mode === classMode || item.fitness_package_type.type === 'Classic Class'
      );
      flattenData.sapienPricings = flattenData.sapienPricings.filter(
        (item) => item.mode === classMode || item.fitness_package_type.type === 'Classic Class'
      );
            ...
            <tbody>
              {(classMode === 'Online' || classMode === 'Hybrid') && (
                <tr className="text-center">
                  ...
                  {pricing.map((item, index: number) => {
                    return <td key={index}>{ptOnlineClasses * (item.duration / 30)} Classes</td>;
                  })}
                </tr>
              )}
              {(classMode === 'Offline' || classMode === 'Hybrid') && (
                <tr className="text-center">
                  ...
                  {pricing.map((item, index: number) => {
                    return <td key={index}>{ptOfflineClasses * (item.duration / 30)} Classes</td>;
                  })}
                </tr>
              )}
              {(classMode === 'Online' || classMode === 'Hybrid') && (
                <tr className="text-center">
                  <td>
                    <img src="/assets/Group-Online.svg" alt="personal-training-online" />
                  </td>
                  {pricing.map((item, index: number) => {
                    return <td key={index}>{groupOnlineClasses * (item.duration / 30)} Classes</td>;
                  })}
                </tr>
              )}
              {(classMode === 'Offline' || classMode === 'Hybrid') && (
                <tr className="text-center">
                  <td>
                    <img src="/assets/Group-Offline.svg" alt="personal-training-online" />
                  </td>
                  {pricing.map((item, index: number) => {
                    return (
              ...
              <tr className="text-center">
                <td>
                  {' '}
                  <img src="/assets/classic.svg" alt="personal-training-offline" />
                </td>
                {pricing.map((item, index: number) => {
                  return <td key={index}>{recorded * (item.duration / 30)} Classes</td>;
                })}
              </tr>
              <tr className="text-center">
                <td>
