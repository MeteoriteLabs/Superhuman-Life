...
export default function ExtendProgram(props: Props): JSX.Element {
    ...
    function onSubmit(form) {
        const duration = form.formData.duration;
        ...
        if (duration === 'Half Yearly') {
            updatePackage({
                variables: {
                    id: fitnessPackageId,
                    data: {
                        Start_date: moment(startDate).add(6, 'M').format(),
                        End_date: moment(endDate).add(6, 'M').format()
                    }
                }
            });
            ...
            updatePackage({
                variables: {
                    id: fitnessPackageId,
                    data: {
                        Start_date: moment(startDate).add(1, 'year').format(),
                        End_date: moment(endDate).add(1, 'year').format()
                    }
                }
            });
        } else {
            updatePackage({
                variables: {
                    id: fitnessPackageId,
                    data: {
                        Start_date: moment(startDate).add(3, 'M').format(),
                        End_date: moment(endDate).add(3, 'M').format()
                    }
                }
            });
        }
