...
const Scheduler = () => {
  ...
  function loadTagData(data: any) {
    ...
    setGroupEndDate(
      moment(flattenData.tags[0].fitnesspackage.End_date).format("YYYY-MM-DD")
    );
    handleRangeDates(
      ...
      for (let j = 0; j < flattenData3.clientPackages.length; j++) {
        if (flattenData3.clientPackages[j].program_managers.length > 0) {
          if (
            ...
            flattenData3.clientPackages[j].program_managers[0]
              .fitnessprograms[0].id
          ) {
  ...
  for (let i = 0; i < userPackage.length - 1; i++) {
    if (userPackage[i].id === userPackage[i + 1].id) {
      if (
        userPackage[i].proManagerFitnessId ===
        userPackage[i + 1].proManagerFitnessId
      ) {
  ...
  if (userPackage.length > 0) {
    programIndex = userPackage.findIndex(
      (item) =>
        item.proManagerId === last[1] && item.proManagerFitnessId === last[0]
    );
  }
  ...
  function handlePrevMonth(date: string) {
    ...
    if (
      moment(date).subtract(30, "days").isSameOrAfter(moment(groupStartDate))
    ) {
                      ...
                      <Col lg={{ offset: 2 }}>
                        ...
                        <Row className="mt-1">
                          <span className="text-capitalize">
                            <b style={{ color: "gray" }}>
                              {tag.client_packages[0]?.length === "N/A"
                                ? 0
