...
export const UPDATE_STARTDATE = gql`
    mutation updateStartDate($id: ID!, $startDate: Date!, $endDate: Date!) {
        updateFitnessprogram(
            input: { where: { id: $id }, data: { start_dt: $startDate, end_dt: $endDate } }
        ) {
            fitnessprogram {
                id
                start_dt
...
export const UPDATE_FITNESSPACKAGE_GROUP_TIME = gql`
    mutation updateFitnesspackageGroupTime(
        ...
        updateFitnesspackage(
            input: {
                where: { id: $id }
                data: { groupstarttime: $groupstarttime, groupendtime: $groupendtime }
            }
        ) {
            fitnesspackage {
                id
...
export const UPDATE_USERPACKAGE_EFFECTIVEDATE = gql`
    mutation updateUserPackage($id: ID!, $effectiveDate: DateTime!) {
        updateUserPackage(input: { where: { id: $id }, data: { effective_date: $effectiveDate } }) {
            userPackage {
                id
                effective_date
            }
