    ...
    function loadData(data: any) {
        ...
        for (var i = 0; i < flattenData.clientPackages.length; i++) {
            if (flattenData.clientPackages[i].program_managers.length !== 0) {
                if (
                    flattenData.clientPackages[i].fitnesspackages[0].fitness_package_type.type ===
                    'Group Class'
                ) {
                    for (
                        var j = 0;
                        j <
                        flattenData.clientPackages[i].program_managers[0].fitnessprograms.length;
                        j++
                    ) {
    ...
    function handleTimeLine(id: any, event: any) {
        ...
        return (
            <VerticalTimelineElement
                ...
                date={`${moment(event.start_time, 'hh:mm a').format('hh:mm a')} - ${moment(
                    event.end_time,
                    'hh:mm a'
                ).format('hh:mm a')}`}
                iconStyle={{
                ...
                icon={<img src={`/assets/${handleIcon(event.tag)}-schedule.svg`} alt="classical" />}
            >
                <div className="text-center">
                    <Row>
                        <Col lg={12} xs={12}>
                            ...
                            {event.tag !== 'Classic' && (
                                <Badge variant={event.mode === 'Offline' ? 'danger' : 'success'}>
                                    {event.mode}
                                </Badge>
                            )}
                        </Col>
