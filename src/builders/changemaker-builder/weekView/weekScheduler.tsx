  ...
  function handleTableRender() {
    ...
    values.forEach((val) => {
      ...
      const startTimeMinute: any = `${
        val.start_time ? (Number(val.start_time.split(":")[0]) < 10 ? `${Number(val.start_time.split(":")[1])}` :  val.start_time.split(":")[1]) :  "0" 
      }`;
      const endTimeHour: any = `${
        val.end_time ? (Number(val.end_time.split(":")[0]) < 10 ? `${Number(val.end_time.split(":")[0])}` :  val.end_time.split(":")[0]) : "0" 
      }`;
      const endTimeMin: any = `${
        val.end_time ?  (Number(val.end_time.split(":")[1]) < 10 ? `${Number(val.end_time.split(":")[1])}` :  val.end_time.split(":")[1]) : "0" 
      }`;
      if (
      ...
      arr[moment(val.session_date).weekday() + 1][startTimeHour][
        ...
        color: "skyblue",
        day: moment(val.session_date).weekday() + 1,
        hour: startTimeHour,
        min: startTimeMinute,
        type: val.type,
        endHour: endTimeHour,
