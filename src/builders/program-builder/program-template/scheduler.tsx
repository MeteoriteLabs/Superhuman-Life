...
const Schedular = (props: any) => {
  ...
  const CREATE_TEMPLATE_SESSION = gql`
    mutation createTemplateSession($day: Int!, $id: ID!) {
      createSession(
        data: { day_of_program: $day, type: "restday", Is_restday: true, changemaker: $id }
      ) {
        data {
          id
        }
  ...
  const CREATE_REST_DAY = gql`
    mutation createRestDay($date: Date!, $id: ID!) {
      createSession(
        data: { session_date: $date, type: "restday", Is_restday: true, changemaker: $id }
      ) {
        data {
          id
        }
  ...
  const GET_SESSIONS_BY_DATE = gql`
    query getprogramdata($date: Date) {
      sessions(filters: { session_date: { eq: $date }, Is_restday: { eq: false } }) {
        data {
          id
          attributes {
            tag
      ...
      flattenData.sessionsBookings
        ...
        .forEach((val) => {
          ...
          arr[calculateDay(props.startDate, val.session.session_date)][startTimeHour][
            ...
            endHour: endTimeHour,
            endMin: endTimeMin,
            id: val.session.activity === null ? val.session.workout.id : val.session.activity.id,
            mode: val.session.mode,
            tag: val.session.tag,
            sessionId: val.session.id,
            activityTarget: val.session.activity === null ? null : val.session.activity_target,
  ...
  function handleRenderTable(data: any) {
    ...
    if (sessions.length) {
      sessions
        ...
        .forEach((val) => {
          console.log(val, val.start_time, val.end_time, val.end_time.split(':')[0],val.end_time.split(':')[1] );
          const startTimeHour: any = `${
  ...
  function handleDuplicate(e: any, changedTime: any) {
    ...
    const addedEventDate =
      dates[(duplicatedDay.length === 0 ? e.day : parseInt(duplicatedDay[0].day.substr(4))) - 1];
                ...
                <Col>
                  {d.type !== 'exercise' ? null : (
                    <p>
                      Weights: <br />
                      <InputGroup className="pt-2">
                        <FormControl disabled value={d.weights ? d.weights : 'N/A'} />
                      </InputGroup>
                    </p>
                  )}
                </Col>
                ...
                <Col>
                  {d.type !== 'exercise' ? null : (
                    <p>
                      Duration: <br />
                      <InputGroup className="pt-2">
                        <FormControl disabled value={d.duration ? d.duration : 'N/A'} />
                      </InputGroup>
                    </p>
                  )}
                </Col>
                ...
                <Row className="pl-2">
                  {d.type !== 'exercise' ? null : (
                    <>
                      ...
                      <Col lg={3}>
                        <InputGroup className="pt-2">
                          <FormControl disabled value={d.restTime ? d.restTime : 'N/A'} />
                        </InputGroup>
                      </Col>
                    </>
                  )}
            ...
            <div
              ...
              <div
                ...
                <Badge
                  ...
                  style={{
                    display: `${
                      moment().format('Do, MMM YY') === moment(val).format('Do, MMM YY')
                        ? 'block'
                        : 'none'
                    }`
                  }}>
                                    ...
                                    <div
                                      ...
                                      <div
                                        ...
                                        <div className="event-time">
                                          {val.type === 'restday'
                                            ? null
                                            : (val.hour === '0' ? '00' : val.hour) +
                                              ':' +
                                              (val.min === '0' ? '00' : val.min) +
                                              ' - ' +
                                              val.endHour +
                                              ':' +
                                              (val.endMin.toString() === '0' ? '00' : val.endMin)}
                                        </div>
                                      </div>
                                    </div>
                                  );
            ...
            <Modal.Body style={{ maxHeight: '600px', overflow: 'auto' }}>
              ...
              {event.type === 'workout' && (
                <Tabs
                        ...
                        <>
                          ...
                          <hr
                            style={{
                              ...
                              marginBottom: '20px',
                              borderTop: '2px solid grey',
                              display: `${val.mainmovement === null ? 'none' : 'block'}`
                            }}></hr>
                          <Row>
                          ...
                          <hr
                            style={{
                              ...
                              marginBottom: '20px',
                              borderTop: '2px solid grey',
                              display: `${val.cooldown === null ? 'none' : 'block'}`
                            }}></hr>
                          <Row>
                            {val?.cooldown === null ? (
                              ''
                          ...
                          <Row className="pt-3">
                            ...
                            <Col lg={10} className="pl-0">
                              {val.equipment_lists.map((d, index) => {
                                return (
                                  <>
                                    <Badge className="p-2" pill variant="secondary" key={index}>
                                      {d.name}
                                    </Badge>{' '}
                                  </>
                                );
                  ...
                  {arr2.event?.import !== 'importedEvent' && event.sessionDate && (
                            ...
                            <div className="p-3 shadow-sm" style={{ borderRadius: '20px' }}>
                              <Row>
                                ...
                                <Col>
                                  <div>
                                    <span>
                                      {handleSessionBookingStatus(val.Session_booking_status)}
                                    </span>
                                  </div>
                                </Col>
                                <Col>
                                ...
                                <Col>
                                  <Button
                                    variant="danger"
                                    disabled={
                                      val.Session_booking_status === 'Canceled' ? true : false
                                    }
                                    onClick={() => {
            ...
            <Modal.Body>
              ...
              <Row className="pt-3 align-items-center">
                ...
                <Col lg={7}>
                  ...
                  />
                  <span>
                    <i className="fas fa-info-circle"></i> Please select from the drop down
                  </span>
                </Col>
              </Row>
              <Row className="pt-3 align-items-center">
