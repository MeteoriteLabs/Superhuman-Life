...
const Schedular = (props: any) => {
  ...
  const GET_SESSIONS_BY_DATE = gql`
    query getprogramdata($date: Date) {
      sessions(
        filters: { session_date: { eq: $date }, Is_restday: { eq: false } }
      ) {
        data {
          id
          attributes {
      ...
      if (arr2.event?.import === "importedEvent") {
        if (arr2.event?.tag === "Group Class") {
          setEvent({
            ...
            id: arr2.event?.id,
            tag:
              props.classType === "Custom" && arr2.event?.tag === "Group Class"
                ? "Group Class"
                : props.classType,
            ...
            sessionDate: handleGroupConflictCheck(
              arr2.event?.sessionDate,
              props.startDate,
              props.days
            ),
          ...
          setEvent({
            ...
            hour: arr2?.h,
            min: arr2?.m,
            endHour: handleEndTime(
              arr2.event?.hour + ":" + arr2.event?.min,
              arr2.event?.endHour + ":" + arr2.event?.endMin
            )
              .split(":")[0]
              .charAt(1),
            endMin: handleEndTime(
              arr2.event?.hour + ":" + arr2.event?.min,
              arr2.event?.endHour + ":" + arr2.event?.endMin
            ).split(":")[1],
            sessionId: arr2.event?.sessionId,
            sessionDate: moment(props.startDate)
      ...
      flattenData.sessionsBookings
        ...
        .forEach((val) => {
          var startTimeHour: any = `${
            val.session.start_time === null
              ? "0"
              : val.session.start_time.split(":")[0]
          }`;
          ...
          var endTimeHour: any = `${
            val.session.end_time === null
              ? "0"
              : val.session.end_time.split(":")[0]
          }`;
          var endTimeMin: any = `${
            val.session.end_time === null
              ? "0"
              : val.session.end_time.split(":")[1]
          }`;
          if (
    ...
    if (sessions.length > 0) {
      sessions
        ...
        .forEach((val) => {
          ...
          if (
            !arr[calculateDay(props.startDate, val.session_date)][
              startTimeHour
            ][startTimeMinute]
          ) {
            arr[calculateDay(props.startDate, val.session_date)][startTimeHour][
              startTimeMinute
            ] = [];
          }
          arr[calculateDay(props.startDate, val.session_date)][startTimeHour][
  ...
  function handleRestDays(val: any) {
    ...
    if (props.clientSessions) {
      if (props.restDays) {
        for (var k = 0; k < props.restDays.length; k++) {
          if (
            ...
            calculateDay(
              props.startDate,
              props.restDays[k].session.session_date
            )
          ) {
            return "rgba(255,165,0)";
          }
      ...
      for (var i = 0; i < props.restDays.length; i++) {
        if (
          val === calculateDay(props.startDate, props.restDays[i].session_date)
        ) {
          return "rgba(255,165,0)";
        }
      }
  ...
  const [updateFitnessProgramSessions] = useMutation(
    UPDATE_FITNESSPORGRAMS_SESSIONS,
    {
      onCompleted: () => {
        setEvent({});
    ...
    if (availabilitySlots.length > 0) {
      for (var x = 0; x < availabilitySlots.length; x++) {
        if (
          ...
          moment(timeInput.endTime, "hh:mm:ss").isBefore(
      ...
      createSession({
        variables: {
          ...
          session_date:
            duplicatedDay.length === 0
              ? e.sessionDate
              : moment(duplicatedDay[0].day, "Do, MMMM YYYY").format(
                  "YYYY-MM-DD"
                ),
  ...
  async function handleImportedEvent(e: any, mode: any, tag: any) {
    ...
    if (arr2.event.type2 === "transferEvent") {
      if (existingValues.length === 0) {
        ...
        var timeStartTransfer: any = new Date(
          "01/01/2007 " + handleTimeFormat(newEvent.startTime)
        );
        var timeEndTransfer: any = new Date(
    ...
    if (availabilitySlots.length > 0) {
      for (var x = 0; x < availabilitySlots.length; x++) {
        if (
          ...
          moment(newEvent.endTime, "hh:mm:ss").isBefore(
            moment(availabilitySlots[x].endTime, "hh:mm:ss")
          )
        ) {
          availabilitySlots.splice(x, 1);
    ...
    if (
      event.isProgram === false &&
      window.location.pathname.split("/")[1] !== "programs"
    ) {
      if (
        window.location.pathname.includes("custom") &&
        event.tag === "Group Class"
      ) {
        const values = [...props.sessionIds];
  ...
  function handleSaveChanges(e: any, mode: any, tag: any) {
    ...
    if (arr2.event?.import === "importedEvent") {
      ...
      const availability =
        changeMakerAvailability.changemakerAvailabilties.find(
          (x: any) =>
            moment(x.date).format("YYYY-MM-DD") ===
            moment(addedEventDate).format("YYYY-MM-DD")
        );
        ...
        <Row className="pt-2">
          <Col>
            <div
              style={{
                borderRadius: "10px",
                display: `${d.type !== "exercise" ? "none" : "block"}`,
              }}
              className="pt-2 shadow-lg"
            >
              <Row className="pl-4">
                <p className="text-capitalize">
                  {d.type !== "exercise" ? null : d.value}
                </p>
              </Row>
                  ...
                  {d.type !== "exercise" ? null : (
                    <p>
                      ...
                      <InputGroup className="pt-2">
                        <FormControl disabled value={d.reps ? d.reps : "N/A"} />
                      </InputGroup>
                    </p>
                  )}
                </Col>
                ...
                <Col>
                  {d.type !== "exercise" ? null : (
                    <p>
                      Sets: <br />
                      <InputGroup className="pt-2">
                        <FormControl disabled value={d.sets ? d.sets : "N/A"} />
                      </InputGroup>
                    </p>
                  )}
                </Col>
  ...
  function confirm() {
    const values = [...arr];
    if (arr2.event.day) {
      values[arr2.event.day][arr2.event.hour][arr2.event.min].splice(
        arr2.event.index,
        1
      );
    }
    ...
    updateSession({
      variables: {
        ...
        session_date: moment(props.startDate)
    ...
    if (values[arr2.d][arr2.h][arr2.m].length === 0) {
      ...
      var timeStartArr: any = new Date(
        "01/01/2007 " + handleTimeFormat(newEvent.startTime)
      );
      var timeEndArr: any = new Date(
        "01/01/2007 " + handleTimeFormat(newEvent.endTime)
      );
      ...
      for (var l = 0; l < values[arr2.d][arr2.h][arr2.m].length; l++) {
        var startTimeHourArr: any = new Date(
          "01/01/2007 " +
            handleTimeFormat(
              values[arr2.d][arr2.h][arr2.m][l].hour +
                ":" +
          ...
          >
            {handleRestDaysData(index + 1)?.isRestDay ? (
              <Badge
                ...
                variant="danger"
              >
                Remove Rest Day <i className="fa fa-minus"></i>
              </Badge>
            ) : (
              <Badge
                onClick={() => {
                ...
                variant="success"
              >
                Add Rest Day <i className="fa fa-plus"></i>
              </Badge>
            )}
          </div>
        );
      ...
      <>
                        ...
                        {min.map((m, index) => {
                          return (
                            <div
                              ...
                              data-min={m}
                              style={{
                                backgroundColor: `${handleRestDays(d)}`,
                              }}
                              onDrop={(e) => {
                                changedEvent = JSON.parse(
                                  e.dataTransfer.getData("scheduler-event")
                                );
                                handleChange(
                                  changedDay,
                                  changedHour,
                              ...
                              onDragLeave={(e) => {
                                ...
                                changedMin =
                                  e.currentTarget.getAttribute("data-min");
                              }}
                            >
                              {arr[d][h][m] &&
                                arr[d][h][m]?.map((val, index: number) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      onClick={() => {
                                        setEvent(val);
                                        setClickedSessionId(val.sessionId);
                                      }}
                                      id="dragMe"
                                      className="schedular-content draggable"
                                      draggable={
                                        val.type === "restday" ? false : true
                                      }
                                      onDragStart={(e) => {
                                      ...
                                      style={{
                                        ...
                                        width: `${
                                          val.type === "restday"
                                            ? "100%"
                                            : `${100 / arr[d][h][m].length}%`
                                        }`,
                                        border:
                                          "2px solid rgba(255,255,255,0.5)",
                                        paddingTop: `${
                                          handleHeight(val) > 15 ? "3" : "2"
                                        }px`,
                                        minWidth: "50% !important",
                                        maxWidth: "50% !important",
                                        cursor: "pointer",
                                        left: `${
                                          index * (100 / arr[d][h][m].length)
                                        }%`,
                                      }}
                                    >
                                      <div
              ...
              <Row>
                <Col lg={6}>
                  ...
                  <Row className="pt-3 align-items-center">
                    ...
                    <Col lg={6}>
                      <FormControl
                        value={
                          event.sessionDate && props.type !== "day"
                            ? moment(event.sessionDate).format("Do, MMM YY")
                            : `Day - ${event.day}`
                        }
                        disabled
                      />
                ...
                <Col lg={6}>
                  <TimeField
                    ...
                    startTime={event.hour + ":" + event.min}
                    disabled={edit}
                  />
                </Col>
              </Row>
              {window.location.pathname.split("/")[1] !== "programs" &&
                (tag || event.tag) !== "Classic" && (
                  <Row className="pt-3 align-items-center">
                    <Col lg={1}>
                      <h6>Mode: </h6>
                    </Col>
                    <Col lg={4}>
                      <Form.Control
                        value={mode === "" ? event.mode : mode}
                        as="select"
                        disabled={
                          props.classType === "Custom" &&
                          arr2.event?.tag === "Group Class"
                            ? true
                            : false
                        }
                        onChange={(e) => {
                        ...
                        <>
                          ...
                          <hr
                            style={{
                              ...
                              borderTop: "2px solid grey",
                              display: `${
                                val.mainmovement === null ? "none" : "block"
                              }`,
                            }}
                          ></hr>
                          <Row>
                            {val?.mainmovement === null ? (
                              ""
                            ) : (
                              <Col className="pt-2">
                                <h5>
                                ...
                                <Row>
                                  <Col>
                                    <div className="text-center">
                                      ...
                                      <div>{val.client?.username}</div>
                                    </div>
                                  </Col>
                                  <Col>
                                    <div>
                                      <span>
                                        {handleSessionBookingStatus(
                                          val.Session_booking_status
                                        )}
                                      </span>
                                    </div>
                                  </Col>
                                  ...
                                  <Col>
                                    <Button
                                      ...
                                      onClick={() => {
                                        handleChangeBookingStatus(val.id);
                                      }}
                                    >
                                      Cancel
                                    </Button>
        ...
        }
        {
          ...
          >
            <Modal.Body>
              ...
              <Row className="pt-3 align-items-center">
                ...
                <Col lg={7}>
                  ...
                  />
                  <span>
                    <i className="fas fa-info-circle"></i> Please select from
                    the drop down
                  </span>
                </Col>
              </Row>
            ...
            <Modal.Footer>
              ...
              <Button
                disabled={
                  document.getElementById("timeErr")
                    ? true
                    : duplicatedDay.length === 0
                    ? true
                    : false
        ...
        }
        {
          ...
          >
            ...
            <Modal.Body>
              <span>
                There is already an existing session at this time. Cannot add!
              </span>
            </Modal.Body>
            <Modal.Footer>
              <Button
        ...
        {
          <Modal
            show={groupDropConflict}
            onHide={() => setGroupDropConflict(false)}
            centered
            backdrop="static"
          >
            <Modal.Header>
