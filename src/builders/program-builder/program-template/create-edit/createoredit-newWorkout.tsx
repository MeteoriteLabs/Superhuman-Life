...
function CreateEditNewWorkout(props: any, ref: any) {
	...
	const GET_SESSIONS_BY_DATE = gql`
		query getprogramdata($date: Date) {
			sessions(
				filters: { session_date: { eq: $date }, type: { ne: "restday" } }
			) {
				data {
					id
					attributes {
		...
		if (frm.day) {
			...
			if (window.location.pathname.split("/")[1] === "programs") {
				...
				data = {
					...
					type: eventJson.type,
					day_of_program: eventJson.day,
					session_date: moment(frm.day[0].day, "Do, MMM YY").format(
						"YYYY-MM-DD"
					),
					changemaker: auth.userid,
					isProgram: false,
	...
	function UpdateProgram(frm: any) {
		...
		if (frm.addWorkout.AddWorkout === "Build") {
			...
			if (Object.keys(frm.addWorkout.mainmovement)[0] === "exercise") {
				frm.addWorkout.mainmovement = JSON.parse(
					frm.addWorkout.mainmovement.exercise
				);
			} else {
				frm.addWorkout.mainmovement.type = Object.keys(
					frm.addWorkout.mainmovement
				)[0];
				frm.addWorkout.mainmovement.value =
					frm.addWorkout.mainmovement[
						Object.keys(frm.addWorkout.mainmovement)[0]
					];
				delete frm.addWorkout.mainmovement[
					Object.keys(frm.addWorkout.mainmovement)[0]
				];
		...
		createWorkout({
			variables: {
				...
				warmup:
					frm.addWorkout.AddWorkout === "Build" ? frm.addWorkout.warmup : null,
				mainmovement:
